...
class ProfileViewController: UIViewController {
    ...
    let nameLabel = UILabel(text: "Alicia Keys", font: .systemFont(ofSize: 20, weight: .light))
    let aboutMeLabel = UILabel(text: "You have opportunity to chat with the best man in the world", font: .systemFont(ofSize: 16, weight: .light))
        ...
        self.dismiss(animated: true) {
            FirestoreService.shared.createWaitingChat(message: message, receiver: self.user) { (result) in
                ...
                case .failure(let error):
                    UIApplication.getTopViewController()?.showAlert(with: "Success!", and: error.localizedDescription)

                }
            }
            UIApplication.getTopViewController()?.showAlert(with: "Message Sent", and: "Success")
        }
    }
}
